	    ...
	    <section class="area-box">
	        <section class="area-box-content">
	            <div class="area-box-title">
	                <span>省，直辖市，地区</span>
	                <a href="javascript:;"><i class="iconfont icon-china"></i></a>
	            </div>
	            <div class="area-box-body">
	                <div class="area-box-item" v-for="item in province">
	                    <div class="ct-checkbox-wrapper" :class="{'ct-checkbox-on': item.Name === currName.province}">
	                        <label class="ct-checkbox" @click="selected('province', item)" :class="{'ct-checkbox-checked': (item.state === 1 || item.state === 3), 'ct-checkbox-indeterminate': item.state === 2}">
	        ...
	        <section class="area-box-content">
	            <div class="area-box-title">
	                <span>当前省下的所有市</span>
	                <a href="javascript:;"><i class="iconfont icon-city"></i></a>
	            </div>
	            <div class="area-box-body">
	                <div class="area-box-item" v-for="item in city">
	                    <div class="ct-checkbox-wrapper" :class="{'ct-checkbox-on' : item.Name === currName.city}">
	                        <span class="ct-checkbox" @click="selected('city', item)" :class="{'ct-checkbox-checked': (item.state === 1 || item.state === 3), 'ct-checkbox-indeterminate': item.state === 2}">
	                            <span class="ct-checkbox-inner"></span>
	                            <input type="checkbox" class="ct-checkbox-input" value="on">
	                        </span>
	        ...
	        <section class="area-box-content">
	            <div class="area-box-title">
	                <span>当前市下的所有县</span>
	                <a href="javascript:;"><i class="iconfont icon-quyudaili"></i></a>
	            </div>
	            <div class="area-box-body">
	                <div class="area-box-item" v-for="item in region">
	        ...
	        <section class="area-box-content">
	            <div class="area-box-title">
	                <span>已选择地区</span>
	                <a class="edit-button" href="javascript:;"><i class="iconfont icon-china"></i></a>
	            </div>
	            <div>
	                <ul class="ct-select-selection__rendered">
	                    <li unselectable="unselectable" class="ct-select-selection__choice" style="user-select: none;" v-for="item in selectedArea">
...
<script type="text/javascript">
	...
	function getObject(obj,key,value){ //返回数据
		let i,j,result='';
	...
	function setData(value){//state 0 1
		var len,
		pName,
		cName,
		pID,
	...
	function typeStateChange(type,name){ //name上一级名称 
		var s,arr,arr2,i,j,state=0,len,stateNum;
		arr = area[type][name];
		len = arr.length;
		stateNum=0;
	...
	export default {
		...
		methods: {
			...
			setDateInit(data){
				...
				if(len === 2){
					...
					for(n in area.city[data.Name]){
						$.each(area.region[area.city[data.Name][n].Name],function(index,value){
							value.state = 3;
						});
					}
				}	
			...
			changeState(type,value){ // 0,1,2,3 无 选中 半选 被动选中
			...
			clearAll(){
				...
				this.selectedArea.map(function(item, index) {
					...
					switch(value.ID.length) {
						case 2: _this.changeState('province', value);
								break;
						case 4: _this.changeState('city', value);
								break;
						case 6: _this.changeState('region', value);
								break;
						default: '';
					}
				});
